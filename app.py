...
Reading: {reading_data.get(f'reading_text_{self.target_language.lower()}', '')[:200]}"""
        ...
        try:
            ...
            return {
                "reading_exercises": [{"question": "Sample question 1"}],
                "writing_exercises": [{"question": "Sample writing prompt"}],
                "listening_exercises": [{"question": "Sample listening question"}],
                "speaking_exercises": [{"question": "Sample speaking prompt"}]
            }
...
Exercises: {json.dumps(lesson_data, ensure_ascii=False)[:500]}"""
        ...
        try:
            ...
            return {
                "reading_exercises": [{"answer": "Sample answer", "explanation": "Explanation"}],
                "writing_exercises": [{"answer": "Sample response", "explanation": "Model answer"}],
                "listening_exercises": [{"answer": "Key points", "explanation": "Listening key"}],
                "speaking_exercises": [{"answer": "Discussion points", "explanation": "Speaking guide"}]
            }
...
*{verse_data.get('verse_text_spanish', verse_data.get('verse_text_english', 'N/A'))}*

**Meditation:**
{verse_data.get('meditation_spanish', verse_data.get('meditation_english', 'N/A'))}

---

# 📚 Reading Comprehension

{reading_data.get('reading_text_spanish', reading_data.get('reading_text_english', 'N/A'))}
...
def generate_lesson(api_key, language, level, model, progress=gr.Progress()):
    ...
    try:
        ...
        lesson_content, pdf_path = system.run_full_lesson_generation(level, progress)
        
        display_text = format_lesson_display(lesson_content)
        
        return display_text, pdf_path
...
with gr.Blocks(title="Bible Language Learning System", theme=gr.themes.Soft()) as demo:
    ...
    Each lesson includes reading, writing, listening, and speaking exercises with complete answer keys.
    """)
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            ### 💰 Estimated Cost:
            - gpt-4o: ~$0.10-0.20/lesson
            - gpt-4o-mini: ~$0.01-0.02/lesson
            - gpt-3.5-turbo: ~$0.002/lesson
            """)
    ...
    generate_btn.click(
        ...
        outputs=[lesson_output, pdf_output]
    )
    ...
    Each lesson is tailored to your language level and includes all four skill areas: reading, writing, listening, and speaking.
    
    **Note:** You need your own OpenAI API key to use this app. Get one at [platform.openai.com](https://platform.openai.com/).
    """)
